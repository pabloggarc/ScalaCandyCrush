...
class Operaciones (numFila: Int, numCol: Int, numColores: Int) {
    ...
    def actualizarPuntos(puntos: Int, borrados: Int, caramelo: Int, modo:  Int): Int = {
        val puntosNuevos = caramelo match {
            case '7' => 5
            case '8' => 10
            case '9' => 15
            case _ => 0
    ...
    def mostrarTablero(tablero: List[Char], vidas: Int, m: Int, puntos: Int): String = {
    ...
    private def longitudCaminos(tablero: List[Char], posicion: Int): List[List[Int]] = {
        posicion match {
            ...
            case _ if tablero(posicion).toInt - '0'.toInt <= 6 => List(posicion, longitud(buscarCamino(tablero, posicion))) :: longitudCaminos(tablero, posicion+1)
            case _ => {
